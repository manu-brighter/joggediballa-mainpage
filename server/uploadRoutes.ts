...
function parseMultipartFormData(body: Buffer, contentType: string): { fileBuffer: Buffer | null; fileName: string; fileMimeType: string } {
  ...
  try {
    ...
    req.on("end", async () => {
      try {
        ...
        const { fileBuffer, fileName, fileMimeType } = parseMultipartFormData(body, contentType);
