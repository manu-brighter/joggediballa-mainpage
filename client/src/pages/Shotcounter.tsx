...
export default function Shotcounter() {
  ...
  const updateScoreMutation = trpc.shotcounter.updateScore.useMutation({
    onMutate: async ({ teamId, amount }) => {
      ...
      const previousTeams = utils.shotcounter.getTeams.getData({
        year: currentYear,
      });
    ...
    onError: (error, _, context) => {
      if (context?.previousTeams) {
        utils.shotcounter.getTeams.setData(
          { year: currentYear },
  ...
  const canEditShotcounter =
    user && ['admin', 'maintainer', 'editor'].includes(user.role);
            ...
            {teams.map((team, index) => (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-2 md:gap-3 min-w-0 flex-1">
                    ...
                    >
                      {index === 0 ? (
                        <Crown
                          className={
                            beamerRowHeight <= 50 ? 'h-3 w-3' : 'h-5 w-5'
                          }
                        />
                      ) : (
                        index + 1
                    ...
                    <span
                      className={cn('font-bold truncate', getRowFontSize())}
                    >
                      {team.name}
                    </span>
                  </div>
                  ...
                  <span
                    className={cn(
                      'font-black tabular-nums text-primary flex-shrink-0',
                      getScoreFontSize(),
                    )}
                  >
                    {team.score}
              ...
              >
                <div className="flex items-center justify-between gap-2 md:gap-4">
                  ...
                  <div className="flex items-center gap-2 md:gap-4 min-w-0 flex-1">
                    ...
                    >
                      {index === 0 ? (
                        <Crown className="h-4 w-4 md:h-6 md:w-6" />
                      ) : (
                        index + 1
                      )}
                    </div>
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Bist du sicher, dass du das Team "{teamToDelete?.name}" löschen
