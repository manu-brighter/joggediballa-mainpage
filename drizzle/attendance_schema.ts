import { mysqlTable, int, varchar, text, mysqlEnum, boolean, timestamp, date, index, uniqueIndex } from "drizzle-orm/mysql-core";
 ...
 */
export const attendanceMembers = mysqlTable("attendance_members", {
  id: int("id").autoincrement().primaryKey(),
...
export const attendanceRecords = mysqlTable("attendance_records", {
  ...
  status: mysqlEnum("status", ["present", "partial", "absent"]).notNull().default("absent"),
  notes: text("notes"),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
}, (table) => ({
  ...
  uniqueSessionMember: uniqueIndex("unique_session_member").on(table.sessionId, table.memberId),
}));
